...
namespace Tests
{
	public class FundsTransferServiceShould
	{
		...
		public async Task BeSuccessful()
		{
			...
			await fundsTransferService.TransferAsync(
				sourceAccountId,
		...
		public async Task RollsBackOnRepositoryException()
		{
			...
			mockThrowingRepository
				.Setup(
					m => m.UpdateAsync(
						It.Is<Guid>(c => c == destinationAccountId),
						It.IsAny<Account>()))
				.Throws<Exception>();
			mockThrowingRepository
				.Setup(m => m.GetAsync(It.IsAny<Guid>()))
			...
			await Assert.ThrowsAsync<Exception>(
				async () => await throwingFundsTransferService.TransferAsync(
					sourceAccountId,
					destinationAccountId,
					103m));
			mockThrowingRepository.Verify(
				m => m.UpdateAsync(
					It.IsAny<Guid>(),
					It.IsAny<Account>()),
				Times.Exactly(3));
