...
namespace Tests
{
	public class CustomExceptionMiddlewareShould
	{
		...
		public async Task DoTheRightThingWithValidationsException()
		{
			...
			var exception = new ValidationsException(
			...
			var requestDelegateMock = SetupRequestDelegateMock(
				bodyStream,
				exception,
				out var httpContext);
			var loggerMock = new Mock<ILogger<CustomExceptionMiddleware>>();
			var sut = new CustomExceptionMiddleware(
			...
			bodyStream.Seek(
				0,
				SeekOrigin.Begin);
			var text = Encoding.ASCII.GetString(bodyStream.ToArray());
			Assert.Equal(
		...
		private static Mock<ProblemDetailsFactory> SetupProblemDetailsFactoryMock()
		{
			...
			problemDetailsFactoryMock
				.Setup(
					m => m.CreateProblemDetails(
						...
						It.IsAny<string?>()))
				.Returns(
					(HttpContext _, int status, string? type, string? title, string? detail, string? instance) =>
						new ProblemDetails
						{
							Type = type,
							Title = title,
			...
			problemDetailsFactoryMock
				.Setup(
					m => m.CreateValidationProblemDetails(
						It.IsAny<HttpContext>(),
						...
						It.IsAny<string>()))
				.Returns(
					(HttpContext _, ModelStateDictionary modelStateDictionary, int? statusCode,
