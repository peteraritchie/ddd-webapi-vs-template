...
public class OrderBuilder
{
    ...
    {
        if (!dateTime.HasValue) throw new InvalidOperationException();
        if (shippingAddress == null) throw new InvalidOperationException();
        if (!lineItems.Any()) throw new InvalidOperationException();
    }
    ...
    public OrderBuilder WithProduct(string skuText, decimal price, int quantity)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(quantity), quantity, $"Quantity should be greater than 0 and less than {ushort.MaxValue}");
        }
