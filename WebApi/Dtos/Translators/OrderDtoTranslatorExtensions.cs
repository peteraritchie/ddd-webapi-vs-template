...
public static class OrderDtoTranslatorExtensions
{
	...
	public static OrderLineItem ToDomain(this OrderItemDto orderItem)
	{
		...
		{
			throw new ArgumentException($"{nameof(orderItem)}.{nameof(orderItem.UnitQuantity)} should not be null.",
				nameof(orderItem));
		}

		if (orderItem.UnitQuantity is < 1 or >= ushort.MaxValue)
		{
			throw new ArgumentOutOfRangeException(nameof(orderItem), orderItem.UnitQuantity,
		...
		{
			throw new ArgumentException($"{nameof(orderItem)}.{nameof(orderItem.UnitPrice)} should not be null.",
				nameof(orderItem));
		}
